...
async function runBot() {
    ...
    try {
        ...
        botStatus.currentMarket = { 
            title: marketTitle, 
            title_es: marketItem.title_es || "Analizando...", 
        ...
        console.log(`📊 Probabilidad (Claude): ${(prob * 100).toFixed(0)}% | Razón: ${analysis.reason.substring(0, 50)}...`);
        ...
        if (prob >= botStatus.predictionThreshold) {
            ...
            if (signalIndex === -1) {
                ...
                // --- 💎 INYECCIÓN DE SEÑAL (Si no fue auto o para el Dashboard) ---
                if (!autoExecuted) {
                    const signalObj = {
                        id: Date.now(),
                        marketName: marketTitle,
                        marketName_es: marketTitleEs,
                        tokenId: marketItem.tokenId, // 🚨 Clave para evitar duplicados
                ...
                // Si ya existe, actualizamos los datos en vivo sin duplicar la fila
                ...
                console.log(`ℹ️ Actualizando datos de señal existente: ${marketTitle.substring(0, 20)}...`);
            }
        }
