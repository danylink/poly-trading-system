...
function loadConfigFromDisk() {
    try {
        if (fs.existsSync(CONFIG_FILE)) {
            ...
            if (savedConfig.maxCopyDaysForWhales !== undefined) botStatus.maxCopyDaysForWhales = parseInt(savedConfig.maxCopyDaysForWhales) || 15;
...
// ==========================================
// ACTUALIZAR BALANCES REALES - VERSIÓN V2 (pUSD + SDK L2)
// ==========================================
async function updateRealBalances() {
    try {
        // 1. Balance de Gas (POL) - Sigue igual
        ...
        // 2. Balance USDC en MetaMask (Saldo fuera de Polymarket)
        ...
        try {
            ...
            if (Array.isArray(positions) && positions.length > 0) {
                for (const pos of positions) {
                    ...
                    // En V2, el campo 'redeemable' es el estándar
                    const isRedeemable = pos.redeemable === true || pos.status === 'REDEEMABLE';

                    if (isRedeemable) {
                        totalUnclaimed += valorActual;
                        continue;   // Ocultamos del dashboard como pediste
                    }

                    // Lógica de Outcome (YES/NO) mejorada para V2
                    let outcomeVal = "N/A";
                    if (pos.outcome) {
                        outcomeVal = String(pos.outcome).toUpperCase();
                    } else if (pos.assetName) {
                        const name = pos.assetName.toUpperCase();
                        if (name.includes("-YES") || name.endsWith(" YES")) outcomeVal = "YES";
                        else if (name.includes("-NO") || name.endsWith(" NO")) outcomeVal = "NO";
                    }

                    const currentTokenId = pos.asset || pos.token_id || pos.asset_id;

                    // Extracción de inversión y precio de entrada con precisión V2
                    ...
                    // Recuperar Nickname y Tatuaje de Motor
                    const whaleData = botStatus.copiedPositions?.find(p => p.tokenId === currentTokenId);
                    const engineTag = botStatus.positionEngines[currentTokenId] || null;
                    ...
                    botStatus.activePositions.push({
                        ...
                        category: pos.category || (typeof getMarketCategoryEnhanced === 'function' ? getMarketCategoryEnhanced(pos.title || pos.market || "") : "OTHER"),
                        outcome: outcomeVal,
                        engine: engineTag, 
                        sizeCopied: invested,
            ...
            // Sincronización de trades copiados (Cleanup)
            if (typeof cleanupCopiedTrades === 'function') {
                await cleanupCopiedTrades();
            }

        } catch (apiError) {
            console.log("⚠️ Error al obtener posiciones API:", apiError.message);
        }

        // Log aleatorio de balances para monitoreo
        if (Math.random() < 0.25) {
            ...
            console.log(`📊 Balances V2: Cartera Total: $${carteraTotalReal} | Disponible (pUSD): $${polyVal.toFixed(2)} | MetaMask: $${metaMaskVal.toFixed(2)} | Gas: ${botStatus.balancePOL} POL`);
        }
    ...
    if (typeof updateCarteraTotal === 'function') updateCarteraTotal();
}
...
async function refreshWatchlist() {
    try {
        ...
        // ===============================================================
        // 🐋 MACRO WATCHLIST → PARA BALLENAS / COPY TRADING (SOLO MERCADOS CORTOS)
        // ===============================================================
        const MAX_DAYS_FOR_COPY = botStatus.maxCopyDaysForWhales || 15;
        const MAX_HOURS_FOR_COPY = MAX_DAYS_FOR_COPY * 24;

        const macroPool = activeMarkets.filter(m => {
            const vol = parseFloat(m.volume || 0);
            if (vol < 1500) return false;                    // Liquidez mínima
            if (m.hoursLeft > MAX_HOURS_FOR_COPY) return false; // ← FILTRADO DINÁMICO
...
// ==========================================
// CHECK AND COPY WHALE TRADES - VERSIÓN FINAL BLINDADA (27 ABRIL 2026)
// ==========================================
...
async function checkAndCopyWhaleTrades() {
    ...
    try {
        ...
        // 🔥 CHEQUE GLOBAL (aplica tanto a Custom como a Auto)
        if ((botStatus.copyTradingCustomEnabled || botStatus.copyTradingAutoEnabled) &&
            ...
            console.log(`⛔ [COPY LIMIT GLOBAL] Ya tienes ${currentActiveCopied}/${botStatus.maxCopyMarketsCustom} mercados activos. Esperando cierre.`);
            return;
        }
        ...
        if (botStatus.copyTradingAutoEnabled) {
            if (!botStatus.autoSelectedWhales || 
        ...
        const getWhaleDisplayName = (whale) => {
            ...
            return (configWhale && configWhale.nickname) ? configWhale.nickname : whale.address.substring(0, 8) + "...";
        };

        // ==================== PROCESAMIENTO POR BALLENA ====================
        for (const whale of allWhales) {
            try {
                // Recalcular en tiempo real cuántos mercados tiene esta ballena
                ...
                if (limitPerWhale > 0 && copiedFromThisWhale >= limitPerWhale) {
                    console.log(`⛔ [COPY LIMIT PER WHALE] ${getWhaleDisplayName(whale)} ya tiene ${copiedFromThisWhale}/${limitPerWhale} mercados. Saltando.`);
                    continue;
                }
...
// ==========================================
// Configuración de filtros de Copy Trading (con límite de días)
// ==========================================
app.post('/api/settings/copy-filters', (req, res) => {
    const { 
        ...
        autoWhaleCount,
        whalePostPartialTp,
        maxCopyMarketsCustom,
        maxCopyDaysForWhales          // ← NUEVO
    } = req.body;
    ...
    if (maxCopyMarketsCustom !== undefined) 
        botStatus.maxCopyMarketsCustom = parseInt(maxCopyMarketsCustom) || 10;

    // 🔥 NUEVO: Días máximos para copiar ballenas
    if (maxCopyDaysForWhales !== undefined) {
        botStatus.maxCopyDaysForWhales = parseInt(maxCopyDaysForWhales) || 15;
        console.log(`📋 Máx. días para Copy Ballenas cambiado a: ${botStatus.maxCopyDaysForWhales} días`);
    }

    saveConfigToDisk("Copy Filters Actualizados");

    console.log(`📋 Filtros Copy Trading actualizados → Máx. Días Ballenas: ${botStatus.maxCopyDaysForWhales}`);
    ...
    res.json({ 
        ...
        whalePostPartialTp: botStatus.whalePostPartialTp,
        maxCopyMarketsCustom: botStatus.maxCopyMarketsCustom,
        maxCopyDaysForWhales: botStatus.maxCopyDaysForWhales   // ← NUEVO
    });
});
